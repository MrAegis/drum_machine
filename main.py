...
pygame.display.set_caption('Beat Baker')
...
def draw_save_menu(bt_nm, typ):
    pygame.draw.rect(screen, black, [0, 0, WIDTH, HEIGHT])
    menu_text = label_font.render('SAVE MENU: Enter a name for your file', True, white)
    save_btn = pygame.draw.rect(screen, gray, [WIDTH // 2 - 200, HEIGHT * 0.75, 400, 100], 0, 5)
    save_txt = label_font.render('Save Beats', True, white)
    screen.blit(save_txt, (WIDTH // 2 - 75, HEIGHT * 0.75 + 30))
    screen.blit(menu_text, (400, 40))
    exit_btn = pygame.draw.rect(screen, gray, [WIDTH - 200, HEIGHT - 100, 180, 90], 0, 5)
    exit_text = label_font.render('Close', True, white)
    screen.blit(exit_text, (WIDTH - 150, HEIGHT - 75))
    if typing:
        pygame.draw.rect(screen, dark_gray, [400, 200, 600, 200], 0, 5)
    entry_rect = pygame.draw.rect(screen, gray, [400, 200, 600, 200], 5, 5)
    entry_text = label_font.render(f'{bt_nm}', True, white)
    screen.blit(entry_text, (430, 250))
    return exit_btn, save_btn, entry_rect
...
def draw_load_menu(index):
    ...
    loaded_bpm = 0
    pygame.draw.rect(screen, black, [0, 0, WIDTH, HEIGHT])
    menu_text = label_font.render('LOAD MENU: Select a file to load', True, white)
    screen.blit(menu_text, (450, 40))
    load_btn = pygame.draw.rect(screen, gray, [WIDTH // 2 - 200, HEIGHT * 0.75, 400, 100], 0, 5)
    load_txt = label_font.render('Load Beats', True, white)
    screen.blit(load_txt, (WIDTH // 2 - 75, HEIGHT * 0.75 + 30))
    delete_btn = pygame.draw.rect(screen, red, [WIDTH // 2 - 100, HEIGHT * 0.9, 200, 50], 0, 5)
    delete_txt = label_font.render('Delete', True, white)
    screen.blit(delete_txt, (WIDTH // 2 - 45, HEIGHT * 0.9 + 7))
    exit_btn = pygame.draw.rect(screen, gray, [WIDTH - 200, HEIGHT - 100, 180, 90], 0, 5)
    exit_text = label_font.render('Close', True, white)
    screen.blit(exit_text, (WIDTH - 160, HEIGHT - 70))
    loaded_rect = pygame.draw.rect(screen, gray, [190, 90, 1000, 475], 3, 5)
    ...
    return exit_btn, load_btn, delete_btn, loaded_rect, loaded_info
...
while run:
    ...
    beats_text = medium_font.render('Beats in Measure', True, white)
    screen.blit(beats_text, [607, HEIGHT - 130])
    beats_value = label_font.render(f'{beats}', True, white)
    screen.blit(beats_value, [695, HEIGHT - 100])
    ...
    addbeats_text = medium_font.render('+1', True, white)
    subbeats_text = medium_font.render('-1', True, white)
    screen.blit(addbeats_text, (820, HEIGHT - 140))
    screen.blit(subbeats_text, (820, HEIGHT - 90))
    ...
    if load_menu:
        exit_button, load_button, delete_button, load_rect, load_info = draw_load_menu(index)

    # instrument playing loop
    if beat_changed:
        play_notes()
        beat_changed = False
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        # enable/disable beats
        if event.type == pygame.MOUSEBUTTONDOWN and not save_menu and not load_menu:
            for i in range(len(boxes)):
                if boxes[i][0].collidepoint(event.pos):
                    coords = boxes[i][1]
                    clicked[coords[1]][coords[0]] *= -1
        # bottom menu event handler
